...
function hspecseries(){
  ...
  $("#instructions").html("Use the sliders to set the initial and final energy levels. The exhibit will then calculate the observed wavelength and colour.");
  ...
  var sketch = function( p ) {
    ...
    p.setup = function() {
      ...
      $("#controls").append("<br><span>INITIAL</span><br>");
      initial_slider = p.createSlider(1,10,3,1);
      initial_slider.class('superposition_slider small_slider');
      initial_slider.parent('controls');
      initial_slider.input(update);

      $("#controls").append("<br><span>FINAL</span><br>");
      final_slider = p.createSlider(1,10,2,1);
      final_slider.class('superposition_slider small_slider');
      final_slider.parent('controls');
      final_slider.input(update);
    ...
    function update() {
      ...
      if (final<initial) {
        p.textSize(12);
        p.text(wv.toFixed(0)+'nm', x+5,15);
      } else {
        p.textSize(20);
        p.text('FINAL '+p.char(8805)+' INITIAL: NO EMISSION LINE PRODUCED', 100,230);
      }
